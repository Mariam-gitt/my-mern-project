...
function Quiz() {
    ...
    // ... (All your original loadQuiz, checkAnswer, and helper functions remain here)
        ...
        <div className="flex h-screen w-full bg-[#FDFBF7] overflow-hidden">
            ...
            <main className="flex-1 overflow-y-auto p-6">
                ...
                <div className="max-w-[620px] mx-auto">
                    ...
                    {/* Score row - updated with theme borders */}
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                        {[
                            { label: "Correct", val: score, color: "text-green-700" },
                            { label: "Wrong", val: total > 1 ? total - 1 - score : 0, color: "text-red-700" },
                            { label: "Streak 🔥", val: streak, color: "text-black" },
                            { label: "Accuracy", val: total > 1 ? `${Math.round((score / (total - 1)) * 100)}%` : "—", color: "text-black" }
                        ].map((stat, i) => (
                    ...
                    {/* Quiz card and options logic remains exactly as you wrote it */}
                    {/* Knitting Yarn Ball Loader for Quiz loading */}
                    {!error && !quiz && (
                        <div className="flex flex-col items-center justify-center p-20">
                            <div className="w-16 h-16 rounded-full border-4 border-[#F5C754] animate-spin border-t-transparent"></div>
                            <p className="mt-4 font-serif italic text-[#111111]/70">Preparing quiz...</p>
                        </div>
                    )}
                    
                    {quiz && (
                        <div className="quiz-container">
                            {/* ... (Your original quiz UI) */}
                        </div>
                    )}
                </div>
            </main>
